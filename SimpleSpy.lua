...
SimpleSpy2.Name = "SimpleSpy Edit By cayvaii"
...
TextLabel.Text = "นี่เป็นข้อความที่ยาวกว่าเล็กน้อย"
...
function newRemote(type, name, args, remote, function_info, blocked, src, returnValue)
	...
	local log = {
		...
		Blocked = blocked,
		Source = src,
		GenScript = "-- กำลังสร้าง โปรดรอสักครู่... (คลิกเพื่อโหลดซ้ำ)\n-- (หากข้อความนี้ยังคงอยู่ แสดงว่าอาร์กิวเมนต์ระยะไกลอาจยาวมาก)",
	...
	schedule(function()
		...
		if blocked then
			logs[#logs].GenScript = "-- รีโมตนี้ถูกป้องกันไม่ให้เริ่มการทำงานของเซิร์ฟเวอร์โดย SIMPLESPY\n\n"
				.. logs[#logs].GenScript
		end
	end)
...
function genScript(remote, args)
	...
	if #args > 0 then
		if not pcall(function()
			...
			gen = gen
				.. "-- TableToString ล้มเหลว! การเปลี่ยนกลับไปใช้ฟังก์ชันการทำงานแบบเดิม (ผลลัพธ์อาจแตกต่างกันไป)\nlocal args = {"
			if
				not pcall(function()
			...
			then
				gen = gen .. "}\n-- tableToString เดิมล้มเหลว! ไม่สามารถถอดรหัสได้"
			end
		end
	...
	if keyToString then
		if typeof(v) == "userdata" and getrawmetatable(v) then
			return string.format(
				'"<void> (%s)" --[[ข้อมูลที่อาจซ่อนอยู่ (tostring ใน SimpleSpy:HookRemote/GetRemoteFiredSignal คุณต้องยอมรับความเสี่ยงเอง)]]',
				safetostring(v)
			)
		elseif typeof(v) == "userdata" then
...
function formatstr(s, indentation)
	...
	return '"'
		...
		.. (
			reachedMax
				and " --[[ ถึงขนาดสตริงสูงสุดแล้ว เปลี่ยน '_G.SimpleSpyMaxStringSize' เพื่อปรับขนาดสูงสุด ]]"
			or ""
		)
end
...
if not _G.SimpleSpyExecuted then
	local succeeded, err = pcall(function()
		...
		then
			...
			shutdown()
			error(
				"สภาพแวดล้อมนี้ไม่รองรับ method hooks!\n(ช่องโหว่ของคุณไม่สามารถเรียกใช้ SimpleSpy ได้)\nขาดหายไป: "
	...
	if not succeeded then
		warn(
			"เกิดข้อผิดพลาดร้ายแรง SimpleSpy ไม่สามารถเปิดใช้งานได้อย่างถูกต้อง\nกรุณา DM ข้อความแสดงข้อผิดพลาดนี้ไปที่ @exx#9394:\n\n"
				.. tostring(err)
		)
		SimpleSpy2:Destroy()
		hookfunction(remoteEvent.FireServer, originalEvent)
...
newButton("คัดลอกโค้ด", function()
	return "คลิกเพื่อคัดลอกโค้ด"
end, function()
	setclipboard(codebox:getString())
	TextLabel.Text = "คัดลอกเรียบร้อยแล้ว!"
end)

--- Copies the source script (that fired the remote)
newButton("คัดลอกรีโมท", function()
	return "คลิกเพื่อคัดลอกเส้นทางของรีโมท"
end, function()
	if selected then
		setclipboard(v2s(selected.Remote.remote))
		TextLabel.Text = "คัดลอก!"
	end
end)

-- Executes the contents of the codebox through loadstring
newButton("รันโค้ด", function()
	return "คลิกเพื่อรันโค้ด"
end, function()
	local orText = "คลิกเพื่อรันโค้ด"
	...
	if succeeded then
		...
		TextLabel.Text = "การดำเนินการผิดพลาด!"
	end
end)

--- Gets the calling script (not super reliable but w/e)
newButton("รับสคริปต์", function()
	return "คลิกเพื่อคัดลอกสคริปต์การโทรไปยังคลิปบอร์ด\nคำเตือน: ไม่น่าเชื่อถืออย่างยิ่ง ไม่มี == ไม่พบ"
end, function()
	if selected then
...
newButton("ข้อมูลรีโมท", function()
	return "คลิกเพื่อดูข้อมูลฟังก์ชัน"
end, function()
	if selected then
		if selected.Function then
			codebox:setRaw(
				"-- ข้อมูลฟังก์ชันการเรียก\n-- สร้างโดย SimpleSpy serializer\n\n" .. tostring(selected.Function)
			)
		end
		TextLabel.Text = "เสร็จแล้ว! ข้อมูลฟังก์ชั่นที่สร้างโดย SimpleSpy Serializer"
	end
end)

--- Clears the Remote logs
newButton("เคลียร์รีโมท", function()
	return "คลิกเพื่อล้างบันทึก"
end, function()
	...
	codebox:setRaw("")
	selected = nil
	TextLabel.Text = "เคลียร์นีโมทแล้ว!"
end)

--- Excludes the selected.Log Remote from the RemoteSpy
newButton("หยุดดักรีโมที้ (รีโมทน)", function()
	return "คลิกเพื่อยกเว้นรีโมตนี้\nการยกเว้นรีโมตจะทำให้ SimpleSpy เพิกเฉย แต่จะยังสามารถใช้งานได้ต่อไป"
end, function()
	if selected then
		blacklist[selected.Remote.remote] = true
		TextLabel.Text = "หยุดดักรีโมท"
	end
end)

--- Excludes all Remotes that share the same name as the selected.Log remote from the RemoteSpy
newButton("หยุดดักรีโมท (ที่ชื่อเดียวกัน)", function()
	return "คลิกเพื่อยกเว้นรีโมตทั้งหมดที่มีชื่อนี้\nการยกเว้นรีโมตจะทำให้ SimpleSpy เพิกเฉย แต่จะยังสามารถใช้งานได้ต่อไป"
end, function()
	if selected then
...
--- clears blacklist
newButton("ล้างแบล็กลิสต์", function()
	return "คลิกเพื่อล้างบัญชีดำ\nการยกเว้นรีโมตจะทำให้ SimpleSpy เพิกเฉย แต่จะยังสามารถใช้งานได้ต่อไป"
end, function()
	blacklist = {}
	TextLabel.Text = "แบล็คลิสต์เคลียร์แล้ว!"
end)

--- Prevents the selected.Log Remote from firing the server (still logged)
newButton("รีโมทหยุดทำงาน (รีโมทนี้)", function()
	return "คลิกเพื่อหยุดการทำงานของรีโมตนี้\nการบล็อกรีโมตจะไม่ลบออกจากบันทึก SimpleSpy แต่จะไม่ทำให้เซิร์ฟเวอร์เริ่มทำงานต่อไป"
end, function()
	if selected then
		if selected.Remote.remote then
			blocklist[selected.Remote.remote] = true
			TextLabel.Text = "ไม่รวม!"
		else
			TextLabel.Text = "ข้อผิดพลาด! อินสแตนซ์อาจไม่มีอยู่อีกต่อไป ลองใช้ Block (n)"
		end
	end
end)

--- Prevents all remotes from firing that share the same name as the selected.Log remote from the RemoteSpy (still logged)
newButton("รีโมทหยุดทำงาน (รีโมทที่มีชื่อเดียวกัน)", function()
	return "คลิกเพื่อหยุดการทำงานของรีโมตด้วยชื่อนี้\nการบล็อกรีโมตจะไม่ลบออกจากบันทึก SimpleSpy แต่จะไม่ทำให้เซิร์ฟเวอร์เริ่มทำงานต่อไป"
end, function()
	if selected then
...
--- clears blacklist
newButton("ล้างแบล็กลิสต์", function()
	return "คลิกเพื่อหยุดการบล็อกรีโมต\nการบล็อกรีโมตจะไม่ลบออกจากบันทึก SimpleSpy แต่จะไม่ทำให้เซิร์ฟเวอร์เริ่มทำงานต่อไป"
end, function()
	blocklist = {}
	TextLabel.Text = "ล้างรายการบล็อกแล้ว!"
end)

--- Attempts to decompile the source script
newButton("ถอดรหัส", function()
	return "พยายามถอดรหัสสคริปต์ต้นฉบับ\nคำเตือน: ไม่น่าเชื่อถืออย่างยิ่ง ไม่มี == ไม่พบ"
end, function()
	if selected then
...
newButton("ปิดข้อมูลรีโมท", function()
	return string.format(
		"[%s] ข้อมูลฟังก์ชั่นสลับ (เพราะอาจทำให้เกิดความล่าช้าในบางเกม)",
...
newButton("บล็อกอัตโนมัติ", function()
	return string.format(
		"[%s] [เบต้า] ตรวจจับและแยกการโทรระยะไกลที่เป็นสแปมออกจากบันทึกอย่างชาญฉลาด",
...
newButton("แสดงสคริปต์ที่เรียกฟังก์ชันนี้", function()
	return string.format(
		"[%s] [ไม่ปลอดภัย] ใช้ 'getcallingscript' เพื่อรับสคริปต์การเรียกสำหรับ Decompile และ GetScript เชื่อถือได้มากกว่ามาก แต่เปิด SimpleSpy เพื่อตรวจจับและ/หรือความไม่เสถียร",
...
newButton("แปลงคีย์ให้เป็นข้อความ", function()
	return string.format(
		"[%s] [เบต้า] ใช้ฟังก์ชันทดลองใหม่เพื่อจำลองพฤติกรรมของ Roblox เมื่อใช้ประเภทที่ไม่ใช่แบบดั้งเดิมเป็นคีย์ในตาราง ยังอยู่ในการพัฒนาและอาจไม่สามารถสะท้อนถึงสตริงได้อย่างถูกต้อง (ว่างเปล่า)",
...
newButton("สลับตั้งค่าที่รีโมทส่งกลับมา", function()
	return string.format(
		"[%s] [ทดลอง] เปิดใช้งานการบันทึกค่าที่ส่งคืนสำหรับ 'GetReturnValue'\n\nใช้วิธีนี้โดยยอมรับความเสี่ยงเอง เนื่องจากสามารถตรวจพบได้",
...
newButton("ดึงค่ารีโมทส่งกลับมา", function()
	return "[ทดลอง] หากเปิดใช้งาน 'ReturnValues' สิ่งนี้จะแสดงค่าส่งคืนที่บันทึกไว้สำหรับ RemoteFunction (ถ้ามี)"
end, function()
	if selected then
...
local frame = game:GetService("CoreGui")["SimpleSpy Edit By cayvaii"].Background
local UserInputService = game:GetService("UserInputService")
...
ScreenGui.Parent = game:GetService("CoreGui")["SimpleSpy Edit By cayvaii"]
ScreenGui.Name = "RimuraHub | SimpleSpy"

Button.Parent = ScreenGui
Button.Size = UDim2.new(0, 60, 0, 60)
Button.Position = UDim2.new(0.5, -557, 0.5, -261)
Button.Image = "rbxassetid://112077568458236"
